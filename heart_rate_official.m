function [heart_rate] = heart_rate_official(data, fs, threshold_1, threshold_2, threshold_3, pos_deviance_threshold, neg_deviance_threshold, sample_size, shouldPlot)  
%TO DO -
% Uncomment the line:
...
R_peak_indices_combined = R_peak_indices(1:num_cols_indices); % REPLACE THIS WITH A ZEROS ARRAY
    ...
    function [meets_deviance_req] = meets_deviance_threshold(hr_value, signal_level)
    ...
    function [indices, noise_lvl, signal_lvl] = dualThreshold(threshold, indices, channel)
        ...
        for index=1:length(R_peak_vals)
           ...
           if (R_peak_vals(index) * max_voltage > threshold) 
               ...
               if (~meets_deviance_threshold(R_peak_vals(index), signal_lvl))
